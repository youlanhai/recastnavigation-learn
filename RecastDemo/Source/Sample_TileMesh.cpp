...
///构建tiles的入口
///原理：将整个地形分割为若干个tile，则整个地形的路点计算，转换到了一个tile的路点计算。
///将每个tile再次分割为若干个很小的单元格（cell），每个单元格在xz平面上，表示一个可走的状态，
///将地形的三角形分配到对应的cell中，每个cell裁减出属于其范围内的三角形部分，然后计算裁减部分的AABB，
///用这个AABB代替三角形。此时，一个cell在y方向上，由若干个AABB组成。很显然，AABB内部是不可以穿过的，
///也就是说，我们得到了一系列障碍AABB。而AABB直接的间隙，则是可以通行的部分，一个间隙区域叫做一个span。
///计算出这些间隙，此时，一个cell就由若干span组成。
///每个cell最多可以移动到周围4个(上、下、左、右)cell上，也就是说，每个span最多可以有4个邻接的span，
///这4个邻接的span分别位于邻接的cell中。通过遍历邻接cell中的span，按人物高度以及可攀爬的高度进行过滤，
///最终可以计算出其邻接的span。
///此时，span与span之间连接成若干张span网。对于每个span网，沿着边缘，可描绘出一个多边形轮廓曲线。
///将轮廓曲线进行分割，得到一组三角形。每组三角形按照最优方式进行合并，得到符合条件的多边形，
///此多边形的集合就是一个PolyMesh。为了便于搜索，将公共边相关联的两个多边形连接起来，这样很容易通过边找到
///相邻的多边形。
///TODO：完善原理。
void Sample_TileMesh::buildAllTiles()
{
    ...
    //计算总的tile数量。tileSize为一tile的尺寸，尺寸的单位为cell。
	const int ts = (int)m_tileSize;
	const int tw = (gw + ts-1) / ts;
	const int th = (gh + ts-1) / ts;
	const float tcs = m_tileSize*m_cellSize;//一个tile真实的尺寸
...
unsigned char* Sample_TileMesh::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
	...
	m_cfg.walkableHeight = (int)ceilf(m_agentHeight / m_cfg.ch);//转换为cell单位
    ...
    //向周围扩张一个边界，以便于正确处理邻接关系。m_cfg.borderSize*m_cfg.cs为边界的真实宽度。
	...
	for (int i = 0; i < ncid; ++i)
	{
        ...
        //根据三角形法线的角度，标记三角形是否可走。
		memset(m_triareas, 0, nctris*sizeof(unsigned char));
		rcMarkWalkableTriangles(m_ctx, m_cfg.walkableSlopeAngle,
								verts, nverts, ctris, nctris, m_triareas);
		
        //栅格化三角形，将三角形添加到此高度域（height field）对应的跨度（span）中
    ...
    //将Heightfield转换成CompactHeightfield
    ...
    //根据可走半径，来缩减可走区域。可防止在边界碰到障碍物而导致卡住。
