...
struct rcEdge
{
	...
	unsigned short polyEdge[2];//边在两个多边形中的分别索引
	unsigned short poly[2];//关联的两个多边形
};
							   ...
							   const int nverts, const int vertsPerPoly)
{
	...
	int maxEdgeCount = npolys*vertsPerPoly;//最大边数。n个顶点，即有n个边
    ...
    //所有多边形的环绕方向一致。如果有两个多边形存在公共边，
    //那么必定边的方向是相反的，即一个是ab，另一个必定是ba。
    //此时，必定是a>b，和b<a的情况。
    //一下两次循环，充分利用了这一特性。
	
    //第一次遍历。记录下索引v0<v1的每条边。
    //遍历所有的多边形。只记录下v0<v1的情况，在第二次遍历的时候会处理v0>v1的情况。
		...
		for (int j = 0; j < vertsPerPoly; ++j)
		{
			...
			if (v0 > v1)
			{
				for (unsigned short e = firstEdge[v1]; e != RC_MESH_NULL_IDX; e = nextEdge[e])
				{
                    ...
                    //vert[0]始终记录较小的索引，vert[1]记录较大的。
                    //edge.poly[0] == edge.poly[1]表示是同一个多边形
	...
	for (int i = 0; i < edgeCount; ++i)
	{
		...
		if (e.poly[0] != e.poly[1])//说明是一个公共边，被两个多边形共享。
		{
			unsigned short* p0 = &polys[e.poly[0]*vertsPerPoly*2];
			unsigned short* p1 = &polys[e.poly[1]*vertsPerPoly*2];
			p0[vertsPerPoly + e.polyEdge[0]] = e.poly[1];//记录了，这条边所连接的多边形id
							 ...
							 const int nvp)
{
	...
	int dx = (int)verts[va*3+0] - (int)verts[vb*3+0];//x方向长度
	int dy = (int)verts[va*3+2] - (int)verts[vb*3+2];//z方向长度
	
	return dx*dx + dy*dy;//边长度的平方
}
...
static bool canRemoveVertex(rcContext* ctx, rcPolyMesh& mesh, const unsigned short rem)
{
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		for (int j = 0, k = nv-1; j < nv; k = j++)
		{
			if (p[j] == rem || p[k] == rem)//有一个删除顶点
			{
                ...
                //检查此边是否存在，因为多边形之间会有公共边
				// Check if the edge exists
				bool exists = false;
				for (int m = 0; m < nedges; ++m)
				{
					int* e = &edges[m*3];
					if (e[1] == b)//e[0]=a=rem，若e[1]=b,则此边共享
					{
    ...
    //两个不邻接（边不相同）的多边形，共享同一个删除顶点。在这种情况下，不应该删除该顶点。
	...
	{
		if (edges[i*3+2] < 2)//没有公共边
			numOpenEdges++;
	}

    //至少有两个多边形共享同一删除顶点，且不邻接。
    //在同一个多边形中，一个顶点，至少会有两个边与其关联。
    //两个多边形，共享同一顶点，如果有没有公共边，那么会有4条边与之关联。
    //如果有一个公共边，那么会有3个边与之关联。并且其中只有2个边是孤立的，即=2。
    //因此，>2是针对两个没有公共边多边形。
    //推广到多个多边形，仍然适用。
	if (numOpenEdges > 2)
		return false;
...
bool rcBuildPolyMesh(rcContext* ctx, rcContourSet& cset, const int nvp, rcPolyMesh& mesh)
{
	...
	{
		// Skip null contours.
		if (cset.conts[i].nverts < 3) continue;//跳过无法构成三角形的轮廓

		maxVertices += cset.conts[i].nverts;
		maxTris += cset.conts[i].nverts - 2;//三角形数 = 多边形顶点数 - 2
		maxVertsPerCont = rcMax(maxVertsPerCont, cset.conts[i].nverts);
	}
	
	if (maxVertices >= 0xfffe)//顶点数过多
	{
    ...
    //多边形数组。最多会有maxTris个多边形，每个多边形最多有nvp个顶点，用两个poly的空间来表示一个多边形数据和他的邻接信息。
	...
	for (int i = 0; i < cset.nconts; ++i)//遍历n个轮廓
	{
		rcContour& cont = cset.conts[i];
		
		// Skip null contours.
		if (cont.nverts < 3)//小于三个顶点，不能构成一个多边形
			continue;
...
/*			printf("\tconst float bmin[3] = {%ff,%ff,%ff};\n", cset.bmin[0], cset.bmin[1], cset.bmin[2]);
		...
		{
			int* t = &tris[j*3];
			if (t[0] != t[1] && t[0] != t[2] && t[1] != t[2])//三个点没有重合的
			{
		...
		if (nvp > 3)
		{
			for(;;)
			{
				...
				// Find best polygons to merge.
				int bestMergeVal = 0;
				int bestPa = 0, bestPb = 0;//最佳合并多边形编号
                int bestEa = 0, bestEb = 0;//最佳合并多边形上的边
	...
	if (mesh.borderSize > 0)
	{
		...
		for (int i = 0; i < mesh.npolys; ++i)
		{
			...
			for (int j = 0; j < nvp; ++j)
			{
				if (p[j] == RC_MESH_NULL_IDX) break;//跳过无效点
				...
				if ((int)va[0] == 0 && (int)vb[0] == 0)//x=0
					p[nvp+j] = 0x8000 | 0;

				else if ((int)va[2] == h && (int)vb[2] == h)//z=h
					p[nvp+j] = 0x8000 | 1;

				else if ((int)va[0] == w && (int)vb[0] == w)//x=w
					p[nvp+j] = 0x8000 | 2;

				else if ((int)va[2] == 0 && (int)vb[2] == 0)//z=h
					p[nvp+j] = 0x8000 | 3;
			}
		}
	}
